...
async function initModel(gl, json_path, texture_path) {

    return await fetch(json_path).then((response) => response.json()).then((json_data) => {

        // ========================================================================================
        var modelVertexPositionBuffer = gl.createBuffer();
        var modelVertices = json_data.vertices;
    
        modelVertexPositionBuffer.itemSize = 3;
        modelVertexPositionBuffer.numItems = modelVertices.length / 3;
        ...
        for (var i = 0; i < json_data.faces.length/11; i++) {
            modelIndices.push(json_data.faces[i*11+1], json_data.faces[i*11+2], json_data.faces[i*11+3])
        }
    
        modelVertexIndexBuffer.itemSize = 1;
        modelVertexIndexBuffer.numItems = modelIndices.length;
        ...
        // ========================================================================================
        var modelVertexColorBuffer = gl.createBuffer();
        var modelColors = [];
    
        for (var i = 0; i < modelVertices.length; i++) {
            modelColors = modelColors.concat(0.80, 0.80, 0.80);
        }
    
        modelVertexColorBuffer.itemSize = 3;
        modelVertexColorBuffer.numItems = modelColors.length / 3;
        ...
        var surfaceNormals = computeSurfaceNormals(modelVertices, modelIndices);
        var vertexNormals = computeVertexNormals(modelVertices, modelIndices, surfaceNormals);

        modelVertexNormalBuffer.itemSize = 3;
        modelVertexNormalBuffer.numItems = vertexNormals.length / 3;
        ...
        var modelTextureCoords = json_data.uvs[0];

        modelTextureBuffer.itemSize = 2;
        modelTextureBuffer.numItems = modelTextureCoords.length / 2;
        modelTextureBuffer.texture = loadTexture(gl, texture_path);
        gl.bindBuffer(gl.ARRAY_BUFFER, modelTextureBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(modelTextureCoords), gl.STATIC_DRAW);
        // ========================================================================================

        var model = {
            vertexBuffer:   modelVertexPositionBuffer,
            indexBuffer:    modelVertexIndexBuffer,
            colorBuffer:    modelVertexColorBuffer,
            normalBuffer:   modelVertexNormalBuffer,
            textureBuffer:  modelTextureBuffer
        }
